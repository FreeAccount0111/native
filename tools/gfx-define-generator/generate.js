...
const structMemberRE = /^\s*(const\w*\s*)?([\w[\:\]]+)\s+?(const\w*\s*)?(\w+)(?:\s*[={]?\s*(.*?)\s*}*\s*)?;(?:\s*\/\/\s*@ts-(.*?)$)?/gm;
const structMap = {};
        ...
        if (!memberCap[4].startsWith('_')) {
            ...
            if (directives) {
                if (directives.startsWith('nullable')) {
                    ...
                    let overrides = {};
                    try {overrides = yaml.load(memberCap[6].slice(9));}
